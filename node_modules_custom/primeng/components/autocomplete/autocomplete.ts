import {Component, NgZone, ElementRef, AfterViewInit, AfterViewChecked, DoCheck, Input, Output, EventEmitter, ContentChild, TemplateRef, IterableDiffers, Renderer, forwardRef, Provider} from '@angular/core';
...
export class AutoComplete implements AfterViewInit, DoCheck, AfterViewChecked, ControlValueAccessor {
    ...
    constructor(private el: ElementRef, private domHandler: DomHandler, differs: IterableDiffers, private renderer: Renderer, private zone: NgZone) {
        this.differ = differs.find([]).create(null);
        (<any>window).AutoCompleteComponentRef = {
            zone: this.zone,
            component: this
        };
    }
    ...
    LoadExternalInputData(isCleanAndLoad: boolean = false) {
        if (isCleanAndLoad)
            this.value.length = 0;
        setTimeout(() => {
            if (this.inputValues && this.inputValues.length) {
                for (var i = 0, len = this.inputValues.length; i < len; ++i) {
                    let inputObj = { [this.field]: this.inputValues[i] };
                    this.input.value = '';
    ...
    onTouched(event) {
        if (this.multiple && this.allowNewInput) {
            ...
            let inputObj = { [this.field]: this.input.value };
